...
- **Server** (`apps/sona.server`): ASP.NET Core + EF Core → Azure SQL dev db (AAD auth in `appsettings.Development.json`). Entities in `Data/DbModels/`: `AppUser` (**int PK**, OIDC/HCAID, `AccessLevelId → AccessLevels` — there is **no Role column**), `Patient` (**int PK**), plus not-yet-registered entity files under `Messaging/` (`MessageOut`, `MessageTemplate`) and `Imports/` (`ImportBatch`, `ImportRowError`) whose `DbSet`s are commented out in `Data/ApplicationDbContext.cs`.
...
- ⚠️ **Known latent bug:** `notifyPatientSchema` in `packages/shared/src/schemas.ts` validates `patientId` with `.uuid()`, but real patient ids are int-strings — Task 03 fixes it.
...
- **Migration baseline is broken:** `Data/Migrations/20260811191806_InitialCreate.cs` has an empty `Up()` while the snapshot (and the later `UniqueMrnIndex` migration) assume `AccessLevels`/`AppLogs`/`AppUsers`/`Patients` exist. A fresh database cannot be rebuilt from committed migrations. Task 00 fixes this; other tasks must NOT add migrations until 00 is merged.
- **No `NotificationsController` exists** — the client's `notificationsApi.notifyReady`/`listForPatient` calls 404 today. Task 03 builds it.
- **Client** (`apps/sona.client`): patients list at `src/routes/patients/index.tsx`; full manage page at `src/routes/patients/manage.tsx` (add/edit/soft-delete, **client-side** search by name/MRN). TanStack Form is set up (`src/hooks/form.tsx` + `form-context.tsx`, field components in `src/components/Form/`); `features/patients/components/patient-form.tsx` is the reference form; `features/patients/api/` has create/update/delete mutation hooks + `get-patients.ts` queryOptions; `features/notifications/api/notify-patient.ts` is the reference mutation. Match these patterns exactly.
