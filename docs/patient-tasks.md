...
| 0 | **Frontend unit tests** (Vitest + Testing Library + MSW, CI workflow) | [tasks/12](tasks/12-frontend-unit-tests.md) | No automated tests exist; DoD is compile-only. Regression test for Task 11 comes with it |
...
- [ ] **Frontend unit tests** — Vitest + Testing Library + MSW for `packages/shared` and `apps/sona.client`, `pnpm test` wired into turbo + a CI workflow that finally runs typecheck/build/test; first tests target the schemas and forms behind Task 11 *(prompt: [tasks/12](tasks/12-frontend-unit-tests.md))*
