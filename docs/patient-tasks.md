...
|---|---|---|---|
| 1 | Org hierarchy + user management, **schema + scoping only (8a–8c)** | [tasks/08](tasks/08-org-hierarchy-user-management.md) | Schema changes get riskier as data grows (org backfill, composite MRN index); tenant isolation (8c) must exist before a second org's data enters |
...
- [x] **Organization hierarchy + user management** — Task 08 shipped 2026-09-01 (8a–8e on one branch, one commit per sub-task): `Organization → Site → Department` tables + `UserDepartmentAccess`; `AppUser.Role` (`system_admin | org_admin | staff | unassigned`) replaces `AccessLevels`; `Patient`/`Provider.OrganizationId` backfilled to a seeded "Default Practice"; per-org MRN uniqueness; role policies (`SystemAdmin`/`OrgAdmin`/`AssignedUser`) + org filtering on every patient/provider/notification query (cross-org ⇒ 404); `OrganizationsController` + `UsersController` (invite-first via MSGraph, pending-approval queue); admin UI for users (TanStack Table + Form), org structure (`/organization`), system-admin organizations page (`/organizations`); staff department context (Zustand store) audited as `MessageOut.DepartmentId`. JIT sign-in now lands as `unassigned` — the first `system_admin` must be set directly in the db. ⚠️ Migration not applied to the Azure dev db from this machine (no Azure access) *(prompt: [tasks/08](tasks/08-org-hierarchy-user-management.md))*
- [ ] **SMS via Webex Connect** — "Ready to be seen" sends a real text to the patient's number through the existing `WebexUtil` (currently unregistered dead code); vendor switch Twilio → Webex incl. BAA confirmation *(prompt: [tasks/07](tasks/07-webex-sms-dispatch.md), depends on Task 03 — merged)*
