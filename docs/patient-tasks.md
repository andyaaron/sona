...
| 7 | **Org hierarchy v2 + PatientLookup + AuditLogs** (Division → Organization → Facility → Department, `Department.FacilityType`, drop `Organization.Type`, Divisions admin page; `PatientLookup` becomes the only store of MRNs; `AuditLogs` replaces every `CreateDate`/`ModDate`) — spec'd 2026-09-02, questions resolved same day | [tasks/19](tasks/19-org-hierarchy-v2-and-patient-lookup.md) + [DBML](tasks/19-sona-schema-v2.dbml) | Team review 2026-09-02 aligned the hierarchy with HCA's real structure; multiple MRN sources per patient are expected, so the crosswalk goes in while every patient still has one MRN. Three migrations, three PRs. Cerner, flat-file import and the patient mobile app are out of MVP (`InCerner`, `ImportBatchId`, `IsUsingMobileApp` dropped; import + Device tables shelved; SMS-only dispatch) |
...
- [ ] **Org hierarchy v2 + PatientLookup + AuditLogs** — Part A: Division above Organization (required, backfilled to a seeded default; system_admin admin page), Site → Facility, `Department.FacilityType` (inpatient/outpatient label, no gating), `Organization.Type` removed. Part B: `PatientLookup` keyed `(OrganizationId, AssigningAuthority, Mrn)` replaces `Patients.Mrn` (`mrn` projected from the primary row; search matches any identifier); `InCerner`, `ImportBatchId` and `IsUsingMobileApp` dropped, notify path is SMS-only. Part C: `AuditLogs` records create/edit/delete for every entity and every `CreateDate`/`ModDate` column is dropped (`MessagesOut.CreateDate` → `AttemptedDateTime`), `AppLogs` dropped in favour of Application Insights; column shape pending from the team's other apps *(prompt: [tasks/19](tasks/19-org-hierarchy-v2-and-patient-lookup.md), schema: [DBML](tasks/19-sona-schema-v2.dbml))*
...
- [ ] **Bulk patient import** — **Shelved 2026-09-02** (was deferred 2026-08-31 pending the provider's file format). `ImportBatch`/`ImportRowError` entity classes are deleted in Task 19 Part B and `Patient.ImportBatchId` dropped; the table design is kept as a note at the end of [tasks/19-sona-schema-v2.dbml](tasks/19-sona-schema-v2.dbml). Re-open when import is back on the roadmap *(prompt: [tasks/09](tasks/09-bulk-csv-import.md) — stale, reconcile against the DBML note before starting)*
