...
| 5 | TanStack Table adoption — **blocked** | [tasks/10](tasks/10-tanstack-table.md) | Waiting on the user-provided reusable table component; after 06 merges (replaces its `sortable-header.tsx`) |
...
- [ ] **TanStack Table adoption** — Replace all hand-rolled data tables (patients index, manage, notification history) with a shared `@tanstack/react-table` component built from a user-provided reusable table; supersedes Task 06's `sortable-header.tsx`. **Blocked until the user supplies their table component** *(prompt: [tasks/10](tasks/10-tanstack-table.md))*
