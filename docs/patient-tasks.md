...
- [ ] **Bug: notify/delete feedback on the patient pages** — "Ready to be seen" toasts nothing on success or failure (a 4xx is invisible), patient delete uses native `window.confirm` instead of the shared `ConfirmDialog` and never toasts errors, Add Patient logs a null-`value` select warning *(found 2026-09-02; prompt: [tasks/16](tasks/16-patient-page-feedback-gaps.md))*
- [ ] **`pnpm lint` has never passed** — mobile `expo lint` auto-installs eslint and mutates `package.json`/lockfile/`pnpm-workspace.yaml` then fails on `unrs-resolver`; `packages/*` have no lint script *(found 2026-09-02; prompt: [tasks/17](tasks/17-lint-toolchain.md))*
- [ ] **Providers page role gate (decision + implementation)** — `@TODO: Lock behind user access level`: staff can currently create/edit/deactivate providers client- and server-side; pick admin-only / staff read-only / documented status quo *(found 2026-09-02; prompt: [tasks/18](tasks/18-providers-page-role-gate.md))*
