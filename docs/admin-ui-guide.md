...
- **Interactions:**
  ...
  3. Click the `opie-schedule-count-internal` chip → toggles all `opie-schedule-block--9999-<n>` rows out of the day sheet (client-side only, no request; other rows and the now-marker position are unaffected); click again to bring them back.
...
- **Layout:** page wrapper `patients-page`; toolbar row, then the table.
...
- **Layout:** page wrapper `patients-manage-page`; toolbar → (form card when open) → table.
...
- **Layout:** page wrapper `providers-page`; toolbar → (form card) → table.
...
- **Layout:** page wrapper `users-page`; toolbar → (form card) → pending table (only when non-empty) → users table.
...
- **Layout:** page wrapper `organization-page`; toolbar → sites block → departments block.
...
- **Layout:** page wrapper `organizations-page`; toolbar (`organizations-toolbar`: heading "Organizations" · **Add organization** `organizations-add-button`, toggles Cancel, `aria-expanded`) → form card → table.
