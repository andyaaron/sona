...
7. **Do not touch these without explicit instruction:** `.npmrc` (`node-linker=hoisted` — Expo breaks without it), the `lightningcss` override in `pnpm-workspace.yaml` (NativeWind requirement), `src/routeTree.gen.ts` (sona.client, generated — never hand-edit), `expo-env.d.ts` (generated, committed on purpose).
...
|---|---|---|
| `src/routes/` (web) / `src/app/` (mobile) | Route files (router shell; web providers live in `src/main.tsx`) | Routes compose features; no business logic here |
...
2. **Route files (sona.client)** live in `src/routes/` (`routesDirectory` in `vite.config.ts`; test files beside them are ignored via `routeFileIgnorePattern`). New route = new file there; the plugin picks it up on dev/build.
